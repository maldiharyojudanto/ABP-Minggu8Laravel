...
<body>
    <div class="container">
        <h1>Edit Gudang</h1>
        <div class="card my-4">
            <div class="card-header">Edit detail gudang</div>
            <div class="card-body">
                <form action="/gudang/update/{{$detail->id}}" method="POST">
                    @csrf
                    <div class="form-group">
                        <label>Nama Gudang</label>
                        <input type="text" name="nama_gudang" value="{{$detail->nama_gudang}}" class="form-control" placeholder="Masukkan nama"/>
                        <small class="form-text text-muted">Masukkan nama gudang, dapat berupa string input.</small>
                    </div>

                    <div class="form-group">
                        <label>Alamat Gudang</label>
                        <input type="text" name="alamat_gudang" value="{{$detail->alamat_gudang}}" class="form-control" placeholder="Masukkan nama"/>
    ...
    <div class="d-flex" id="wrapper">
        ...
        <div id="page-content-wrapper">
            ...
            <div class="container-fluid px-4">
                <h3 class="fs-4 mb-3">Edit Gudang</h3>

                <div class="col">
                    <div class="card mb-2">
                        <div class="card-header">Edit Detail {{$detail->nama_gudang}}</div>
                            <div class="card-body">
                                <form action="/gudang/update/{{$detail->id}}" method="POST">
                                    @csrf
                                    <div class="form-group">
                                        <label>Nama Gudang</label>
                                        <input type="text" name="nama_gudang" value="{{$detail->nama_gudang}}" class="form-control" placeholder="Masukkan nama" required/>
                                    </div>

                                    <div class="form-group">
                                        <label>Alamat Gudang</label>
                                        <input type="text" name="alamat_gudang" value="{{$detail->alamat_gudang}}" class="form-control" placeholder="Masukkan nama" required/>
                                    </div>
